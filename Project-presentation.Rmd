...
gplot(comercianti,displaylabels=TRUE, main="Comercianti", gmode="graph")
delete.vertices(comercianti, isolates(comercianti))
gplot(comercianti, displaylabels = TRUE, main="Grupuri de comercianti", gmode="graph")
```
...
gplot(netsym,vertex.cex=0.5,main="Too small nodes",gmode="graph")
gplot(netsym,vertex.cex=6,main="Too large nodes",gmode="graph")
gplot(netsym,vertex.cex=2,main="Just right node size",gmode="graph")
```
...
gplot(netsym,usearrows=FALSE,vertex.cex=4, main="Different node type",
...
gplot(netsym,vertex.cex=1.5,vertex.col="grey25", main="Edge coloring example",
     edge.col=linecol_pal[edge_cat],edge.lwd=2, gmode="graph")
```

# Different edge width
```{r}
widths <- c(2,6,10)
gplot(netsym,vertex.cex=1.5,main="Different edge width",
     edge.lwd=1.5*widths, gmode="graph")
```
...
# Capitolul 9: "Affiliation Networks" - Introducere
...
* nodurile din cadrul lor reprezinta doua clase diferite, fiind realizate legaturi doar intre obiecte de tipuri diferite
...
Similar cu celelalte retele, si "affiliation networks" pot fi create atat pornind de la o matrice de incidenta, cat si de la o lista de muchii.
...
# Capitolul 9: "Affiliation Networks" - Proiectii
...
# Capitolul 9: "Affiliation Networks" - Analiza actori
...
Putem analiza si relatii de dependenta intre datele pe care le extragem. 

* regresie intre numarul de filme in care a jucat un actor si media scorului acestor filme
* *dupa cum se poate vedea in urma analizei urmatoare, precum si din afisarea grafica a rezultatelor, nu se poate extrage o astfel de legatura*

```{r warning=FALSE, echo=FALSE, include=TRUE, results="hide"}
for (i in 161:1365) {
...
# Capitolul 9: "Affiliation Networks" - Analiza filme
Vom analiza mai multe statistici, pentru ca in final sa vizualizam reteaua formata din filmele care au in comun actori cu macar cinci alte filme, din care se poate observa ca fac parte cu preponderenta serii de filme precum: Star Wars, Harry Potter, The Hobbit etc.
...
***Cea mai mare componenta conexa din proiectia filmelor:***
...
***Filmele care au actori in comun cu macar alte cinci filme:***
